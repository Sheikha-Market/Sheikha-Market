...
const SheikhaFinalLaunch = {
    ...
    igniteEmpire: async () => {
        ...
        try {
            ...
            console.log(`   ✅ GitHub: Org=${report.github.organization} | Repos=${report.github.reposCount} | 2FA=${report.github.twoFactor}\n`);
        } catch (e) {
        ...
        if (!keyExists) {
            console.log('💡 [2/2] جلالة الإمبراطور: بانتظار وضع service-account-key.json في جذر المشروع.');
            ...
            try {
                ...
                console.log(`   ✅ Google Cloud: Storage=${cloudResult.storage} | BigQuery=${cloudResult.bigquery} | PubSub=${cloudResult.pubsub}\n`);
            } catch (e) {
        ...
        if (report.status === 'Full_Activation') {
            ...
            console.log('  🛡️  رادار شيخة: الأنظمة محمية — GitHub نشط، Google Cloud ينتظر المفتاح.');
