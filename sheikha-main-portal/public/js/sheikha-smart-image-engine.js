 ...
 */
function resolveSmartUnsplashImage(product) {
  ...
  else if (name.includes('فولاذ') || name.includes('حديد') || name.includes('steel')) bestCategory = 'steel';
  ...
  else if (name.includes('سكراب') || name.includes('خردة') || name.includes('scrap')) bestCategory = 'scrap';
  else if (name.includes('تشليح') || name.includes('سيارة') || name.includes('محرك')) bestCategory = 'salvage';
  ...
  // بناء URL بدقة عالية ومعالجة احترافية
  return buildUnsplashUrl(photoId, {
    ...
    sat: 8, // +8% تشبع (لإبراز اللون المعدني)
    con: 6, // +6% تباين (لوضوح التفاصيل)
    sharp: 12 // +12 حدة (لنقاء الحواف)
  });
}
...
function getDefaultUnsplashImage() {
  ...
  return buildUnsplashUrl(defaultId);
}
 ...
 */
function cacheProductImage(productId, imageUrl) {
  try {
 ...
 */
function getCachedProductImage(productId) {
  try {
    const cache = JSON.parse(localStorage.getItem(CACHE_KEY) || '{}');
...
function getSmartProductImage(product) {
  ...
  const imageUrl = resolveSmartUnsplashImage(product);
  
  // 3. احفظها في الذاكرة المؤقتة
  cacheProductImage(product.id, imageUrl);
